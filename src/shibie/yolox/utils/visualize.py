...
def vis(img, boxes, scores, cls_ids, conf=0.5, class_names=None):
    ...
    for i in range(len(boxes)):
        ...
        txt_size = cv2.getTextSize(text, font, 5, 1)[0]
        pts = []
        for j in range(num_pts):
            pts.append(tuple(np.array(box[(2*j):(2*(j+1))], dtype=np.int32)))
        for j in range(num_pts):
            cv2.line(img, pts[j], pts[(j+1) % num_pts], color, 15)
        ...
        cv2.putText(img, text, (x0, y0 + txt_size[1]), font, 5, txt_color, thickness=5)
        a = int((pts[0][0]+pts[2][0])/2)
        b = int((pts[0][1]+pts[2][1])/2)
        h,w,p=img.shape
        c=int(h/2)
        d=int(w/2)
        x_p=d-a
